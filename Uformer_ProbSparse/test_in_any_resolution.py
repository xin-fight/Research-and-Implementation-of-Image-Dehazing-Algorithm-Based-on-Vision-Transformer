...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        # print(torch.clamp(rgb_restored, 0, 1).size())
        # print(torch.clamp(data_test[0], 0, 1).size())

        ssim_val_rgb2.append(
            utils.SSIM(torch.clamp(rgb_restored, 0, 1).cpu(), torch.clamp(data_test[0], 0, 1).cpu()).item())

        psnr_val_rgb2.append(
            utils.batch_PSNR(torch.clamp(rgb_restored, 0, 1).cpu(), torch.clamp(data_test[0], 0, 1).cpu(),
                             False).item())

        # 把通道C放到最后一维， psnr_loss/ssim_loss需要知道那一维是通道维
        rgb_restored = torch.clamp(rgb_restored, 0, 1).cpu().numpy().squeeze().transpose((1, 2, 0))

        # psnr_loss这输入不能为torch
        psnr_val_rgb.append(psnr_loss(rgb_restored, rgb_gt))
        ssim_val_rgb.append(ssim_loss(rgb_restored, rgb_gt, channel_axis=-1))
...
ssim_val_rgb2 = sum(ssim_val_rgb2) / len(test_dataset)

print("PSNR: %f, SSIM: %f " % (psnr_val_rgb, ssim_val_rgb))
print("PSNR2: %f, SSIM2: %f " % (psnr_val_rgb2, ssim_val_rgb2))
