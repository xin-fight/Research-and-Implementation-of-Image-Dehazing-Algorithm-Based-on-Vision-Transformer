...
for epoch in range(start_epoch, opt.nepoch + 1):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        print(
            f'\r{i}/{Len_train_loader}: loss:{loss.item():.5f} = CharbonnierLoss:{opt.w_loss_CharbonnierLoss * loss_rec.item():.5f}; contrast: {opt.w_loss_vgg7 * loss_vgg7.item():.5f} |all_ap:{all_ap:.5f} all_an:{all_an:.5f} |time_used (Min):{(time.time() - epoch_start_time) / 60 :.1f}',
            end='', flush=True)
    ...
    torch.save({'epoch': epoch,
                'state_dict': model_restoration.state_dict(),
                'optimizer': optimizer.state_dict()
                }, os.path.join(model_dir, "epoch_model_{}.pth".format(epoch)))
    #
