...
class LinearProjection(nn.Module):
    ...
    def forward(self, x, attn_kv=None):
        ...
        q = rearrange(q, ' (b N) c -> b N c', b=B_, N=N)
        ...
        ####################### 为了测试参数修改后的代码，使用torchstat测试的时候，linear输入需要两层
        attn_kv = rearrange(attn_kv, ' b N c -> (b N) c')

        kv = self.to_kv(attn_kv)

        kv = rearrange(kv, ' (b N) c -> b N c', b=B_, N=N)

        kv = kv.reshape(B_, N, 2, self.heads, C // self.heads).permute(2, 0, 3, 1, 4)
        #######################
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        """利用x产生每个head的q，k，v，并计算q @ k.transpose"""
        # qkv(): -> q,k,v各为：[batch_size*num_windows, num_heads, N=Mh*Mw, dim_head]
        # [Bnw, num_head, N, dim]
        q, k, v = self.qkv(x, attn_kv)  # 也是nn.Linear产生的

        q = q * self.scale  # q:[batch_size*num_windows, num_heads, Mh*Mw, dim_head]
        # transpose: -> [batch_size*num_windows, num_heads, embed_dim_per_head, Mh*Mw]
        # @: multiply -> [batch_size*num_windows, num_heads, N=Mh*Mw, N=Mh*Mw]
        ...
        # .size(-1): 取最后一维的维度
        # attn: [batch_size*num_windows, num_heads, Mh*Mw, Mh*Mw] -> Mh*Mw
        ...
        由上：
            ...
            relative_position_bias:             [nH, Mh*Mw, Mh*Mw]  
        r_p_b差第一个维度，因此使用unsqueeze对第一个维度扩充，然后广播机制进行相加
        '''
        # attn q @ k.transpose
        attn = attn + relative_position_bias.unsqueeze(0)  # 公式中+B的过程

        """SW-MSA attn加入mask，窗口内不一定都要一起做attention；W-MSA则不加"""
        if mask is not None:  # 说明是SW-MSA，窗口内不一定都要一起做attention
            # mask: [nW, Mh*Mw, Mh*Mw]
            nW = mask.shape[0]  # num_windows
            mask = repeat(mask, 'nW m n -> nW m (n d)', d=ratio)

            # attn:      [batch_size*num_windows, num_heads, Mh*Mw, Mh*Mw]   * N=Mh*Mw *
        ...
        # ps_atten = self.ProbSpare(x, x, x, relative_position_bias, mask)

        attn = self.attn_drop(attn)

        """attn 与 v 相乘的到最后的attention分数"""
        # attn:[batch_size*num_windows, num_heads, Mh*Mw, Mh*Mw]   V:[batch_size*num_windows, num_heads, Mh*Mw, dim_head]
        # @: multiply -> [batch_size*num_windows, num_heads, Mh*Mw, embed_dim_per_head]  (embed_dim_per_head = dim_head)
        # transpose: -> [batch_size*num_windows, Mh*Mw, num_heads, embed_dim_per_head]
        # reshape: -> [batch_size*num_windows, Mh*Mw, total_embed_dim]
        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)

        ####################### 原代码
        # x = self.proj(x)  # 对多个head输出融合
        #######################
        ...
        x = rearrange(x, ' (b N) c -> b N c', b=B_, N=N)
        #######################
