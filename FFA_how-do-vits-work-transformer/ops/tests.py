...
def test(model, n_ff, dataset,
    ...
    # (2, 2, num_classes, num_classes) 全为0
    ...
    # cms_bin shape: [len(bins)=11 - 1, num_classes, num_classes]
    # cms_bin 从[0., 0.1] ... 有10项，每项（num_classes, num_classes） cm(ys.numpy(), ys_pred.numpy(), filter_min=start, filter_max=end)
    cms_bin = [np.zeros(cm_shape) for _ in range(len(bins) - 1)]
    # conf_acc_bin shape: [len(bins)=11 - 1, ] 全0
    ...
    for step, (xs, ys) in enumerate(dataset):
        ...
        ys_t = ys_t.cpu()  # MinUp后的ys
        ys = ys.cpu()
        ys_pred = ys_pred.cpu()

        ########## B. Measure Confusion Matrices 测量混淆矩阵 ##########
        # torch.sum(-ys_t * F.log_softmax(torch.log(ys_pred), dim=-1), dim=-1).mean()
        ...
        # cutoffs (0.0, 0.9)    cms (2, 2, num_classes, num_classes) 全为0
        # cutoff为某个值； cm_group:(2, num_classes, num_classes)    2表示有real和predict两大类，每类里面有num_classes个值
        for cutoff, cm_group in zip(cutoffs, cms):
            # 混淆矩阵 [num_classes, num_classes]
            cm_certain = cm(ys.numpy(), ys_pred.numpy(), filter_min=cutoff)  # [cutoff, 1]
            cm_uncertain = cm(ys.numpy(), ys_pred.numpy(), filter_max=cutoff)  # [0, cutoff]
        ...
        # cms (2, 2, num_classes, num_classes) 全为0
        # cms [[np.zeros(cm_shape), np.zeros(cm_shape)] for _ in range(len(cutoffs))]  cutoffs: (0.0, 0.9)
        # 2表示有real和predict两大类，每类里面有num_classes个值    [0] + cm_certain   [1] + cm_uncertain
        # gacc： Global accuracy p(accurate). For cm_certain, p(accurate|confident).
        ...
        # cms_bin 从[0., 0.1] ... 有10项，每项（num_classes, num_classes） cm(ys, ys_pred, filter_min=start, filter_max=end)
